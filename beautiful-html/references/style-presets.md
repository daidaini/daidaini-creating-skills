...
- **配色令牌**：深黑底 `#05070d`，深蓝辅助底 `#0b1020`，冷白正文 `#eaf7ff`；信号色为荧光青 `#00f5d4`、品红 `#ff2da6`、黄绿 `#c7ff3d`、琥珀 `#ffc857`。
- **字体角色**：Orbitron 用于标题与大号章节数；Space Mono 用于标签、编号和代码；Noto Sans SC 用于中文长文，避免 Orbitron 被用于正文而降低可读性。
- **结构性装饰**：细密工程网格、轻扫描线、斜置线框/圆形、标题双色错位。装饰不遮挡正文，也不使用持续动画。
- **组件语言**：深蓝面板 + 实色偏移阴影；卡片交替使用青/品红阴影；黄绿承担章节内的标签和流程编号；近黑公式框只用于短代码、流程式或命令。
- **禁则**：不使用蓝紫渐变；不以荧光文字承载整段正文；不让故障效果影响中文阅读；不把每个组件都做成发光边框；不让过密网格压过正文。

...
3. **Generate the page.** Start from [template/skeleton.html](template/skeleton.html): copy it, replace the design-token variables (fonts, colors, decorative shapes) and fill each chapter with the planned content using the component classes. Keep class names stable and legible so colors/content stay easy to modify.
...
- [Style Presets](references/style-presets.md) — the 换装 reference table (Scandinavian pop, Wabi-sabi, Bauhaus, Cyberpunk, Editorial, Swiss, Retro-print) and how to swap style/color/font variables.
- [Component Library](references/component-library.md) — the parts inventory with concrete HTML/CSS recipes for each component.
- [Skeleton Template](template/skeleton.html) — verified single-file starting point that already implements the design language (tokens + all components) with placeholder content.

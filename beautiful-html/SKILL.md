...
**Zen / 侘寂 / 日式极简 / 未来科技** — when the user's request mentions 禅, 侘寂, wabi-sabi, 日式极简, Japanese minimalism, 未来科技, futuristic tech, or similar zen-oriented language, **do not use the beautiful-html skeleton**. Instead, delegate to the `content-to-zen-static-html` skill, which produces a single-file static HTML with a Japanese Minimalism × Wabi-Sabi × Futuristic Tech visual language and zero external dependencies. The beautiful-html skill's design rules (offset shadows, rotated stickers, hollow chapter numbers, Google Fonts) do not apply in this case; the zen skill's own rules govern the output.

Two structural families exist for non-zen presets:
...
- **Zen delegation:** if the request matches 禅 / 侘寂 / wabi-sabi / 日式极简 / 未来科技 / futuristic tech, the beautiful-html skill must not be used. The `content-to-zen-static-html` skill owns the visual rules for those styles. Attempting to apply beautiful-html presets (offset shadows, Google Fonts, sticker badges) to a zen request produces a mismatched result.
...
- [Style Presets](references/style-presets.md) — the 换装 reference table (Scandinavian pop, Wabi-sabi, Bauhaus, Cyberpunk, Editorial, Swiss, Retro-print, Elegant Vintage, Zen) and how to swap style/color/font variables. Elegant Vintage and Zen have their own dedicated sections.
...
- [content-to-zen-static-html](../../content-to-zen-static-html/SKILL.md) — delegated skill for Zen / 侘寂 / 日式极简 / 未来科技 style requests.

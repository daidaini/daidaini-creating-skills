# Trigger Evaluation Report

**Date**: (auto-generated)
**Total cases**: 27
**Passed**: 27
**Failed**: 0
**Pass rate**: 100%

**Triggers in description**: 15
**Triggers in interface.yaml**: 15

## Results

| Input | Expected | Got | Status |
|---|---|---|---|
| 给我一个 Python 的 cheat sheet | True | True | PASS |
| 帮我整理一下重点 | True | True | PASS |
| 我需要 SQL JOIN 的速查表 | True | True | PASS |
| can you give me a quick reference for React hooks | True | True | PASS |
| 给我一个浓缩版的 Kubernetes 知识点总结 | True | True | PASS |
| cram sheet for statistics exam | True | True | PASS |
| one-pager on Docker networking | True | True | PASS |
| 复习资料 - CSS Grid 布局 | True | True | PASS |
| 考前速览：线性代数公式 | True | True | PASS |
| study guide for machine learning interview | True | True | PASS |
| 帮我做一份 Git 操作的备忘录 | True | True | PASS |
| 记忆卡：英语不规则动词 | True | True | PASS |
| show me a cheat sheet for Linux commands | True | True | PASS |
| cheat sheet for everything about programming | True | True | PASS |
| 速查表 | True | True | PASS |
| cheat sheet | True | True | PASS |
| 给我一份 SQL JOIN 速查表，输出 HTML | True | True | PASS |
| React hooks cheat sheet，做成单文件 HTML | True | True | PASS |
| 给我一个详细的 Python 教程 | False | False | PASS |
| deep dive into React performance | False | False | PASS |
| comprehensive tutorial on Docker | False | False | PASS |
| 帮我规划一个系统学习路径 | False | False | PASS |
| 什么是 REST API | False | False | PASS |
| 比较 React 和 Vue 的区别 | False | False | PASS |
| 帮我写一篇关于气候变化的文章 | False | False | PASS |
| 把这篇长文转成单文件 HTML | False | False | PASS |
| 用 content-to-zen-static-html 把笔记做成网页 | False | False | PASS |

## Coverage Gaps


## Recommendations

- ✅ All cases pass — trigger coverage is solid
- Consider adding more near-neighbor negative cases
- Add adversarial test cases (phrases designed to confuse routing)